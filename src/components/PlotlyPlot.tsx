...
const parsePlotCode = (code: string) => {
  ...
  // Try to extract data
  const dataMatch = code.match(RegExp("y\\s*=\\s*\\[(.*?)\\]", "s"));
  if (dataMatch && dataMatch[1]) {
    result.y = dataMatch[1].split(',').map((s) => parseFloat(s.trim()));
  }

  // Try to extract x labels
  const xMatch = code.match(RegExp("x\\s*=\\s*\\[(.*?)\\]", "s"));
  if (xMatch && xMatch[1]) {
...
export const PlotlyPlot = ({ code, width = 800, height = 500 }: PlotlyPlotProps) => {
  ...
  useEffect(() => {
    try {
      ...
      const layout = {
        title: parsed.title,
        xaxis: {
          title: parsed.xaxis.title
        },
        yaxis: {
          title: parsed.yaxis.title
        },
        autosize: true,
        margin: {
          l: 50,
